...
public class EDIScanner {
	...
	public EDIScanner(String input, String output) {
		...
		if (input == null) {
			String filePath = "FLEX-123456-1.edi_backup";
			try {

				inputSource = new InputSource(new FileReader(filePath));
			} catch (Exception e) {
	...
	public void run() {
		...
		try {
			...
			System.out.println();
			System.out.println("Unrecoverable syntax exception: " + syntaxException.getClass()
					.getCanonicalName() + " - " + syntaxException.getMessage());
...
//		String outputFileName = commandLine.getOption("o");
	...
	private class ScanningHandler extends DefaultHandler {
		...
		public void startElement(String namespace, String localName, String qName, Attributes atts)
				throws SAXException {
			String indent;
			if (localName.startsWith(parser.getXMLTags()
					.getInterchangeTag())) {
			...
			for (int i = 0; i < atts.getLength(); i++)
				scannerOutput.println(indent + atts.getLocalName(i) + "=" + atts.getValue(i));
		}
	...
	private class SyntaxExceptionHandler implements EDISyntaxExceptionHandler {
		public boolean process(RecoverableSyntaxException syntaxException) {
			...
			scannerOutput.println("Recoverable syntax exception: " + syntaxException.getClass()
					.getCanonicalName() + " - " + syntaxException.getMessage());
			return true;
		}
	}
